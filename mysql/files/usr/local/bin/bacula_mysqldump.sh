...
main() {
    ...
    mysql "${auth_string[@]}" ${mpass:+-p$mpass} -BNe \
	...
	mysql "${auth_string[@]}" ${mpass:+-p$mpass} -BNe "SHOW GRANTS FOR $line"
    ...
    while read -r db; do
        Dblist+=("$db")
    done < <(mysql "${auth_string[@]}" ${mpass:+-p$mpass} -BNe "SHOW DATABASES")
    IFS="$IFS_BAK"
	...
	if inArray Nobase "${Dblist[i]}"; then
	    ...
	    mysqldump "${auth_string[@]}" ${mpass:+-p$mpass} --routines --single-transaction --skip-dump-date --ignore-table=mysql.event \
