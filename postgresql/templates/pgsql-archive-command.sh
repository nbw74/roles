...
main() {
    ...
    rsync $rsync_opts "${PGARCHIVE}/${BASENAME}.bz2" ${BACKUPUSER}@${BACKUPSERVER}:${BACKUPDIR}/ 2>$LOGERR

    # If no error, then remove compressed file
    if (( ! $? )); then
    ...
    logger -p user.warning -t "$bn" "* ERROR occured in function '$fn' on line ${no}. Command output: '$(cat ${LOGERR:-NOFILE}|awk '$1=$1' ORS=' ')'"
